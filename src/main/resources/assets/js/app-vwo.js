...
(function () {
    var vwo = function () {
            ...
            var bindToggleOnCampaignClick = function(campaigns) {
                for(var i = 0; i < campaigns.length; i++) {
                    let campaignId = campaigns[i].id,
                        campaignElem = $('#vwo-campaign-' + campaignId);

                    campaignElem.on("click", function () {
                ...
                getCampaignsAndShow: function (getCampaignsCallback, getCampaignsErrorCallback) {
                    ...
                    var onSuccessCallback = function (allCampaigns) {
                        ...
                        if(campaigns.length > 0) {
                            ...
                            if (campaigns.length > 0) {
                                $("#campaigns-list-list").html(campaignShortcuts);
                            }
                            /*
                            $('.vwo-campaign')
                            vwoCampaignDetailsManager.toggleCampaignDetails(campaignId);
        ...
        var vwoNewCampaignWizardManager = function () {
            ...
            return {
                ...
                getSelectedCampaignType: function () {
                    return $("input[name='campaignType']:checked").val();
                },
        ...
        var vwoCampaignDetailsManager = function () {
            ...
            var getStartBtn = function (campaignId) {
                    return $('#start-btn-' + campaignId);
                },
                getPauseBtn = function (campaignId) {
                    return $('#pause-btn-' + campaignId);
                },
                getArchiveBtn = function (campaignId) {
                    return $('#archive-btn-' + campaignId);
                },
                getUnarchiveBtn = function (campaignId) {
                    return $('#unarchive-btn-' + campaignId);
                },
                getDeleteBtn = function (campaignId) {
                    return $('#delete-btn-' + campaignId);
                };

            var updateStatusButtons = function (campaignId, status) {
                getStartBtn(campaignId).toggleClass("disabled", status === "running" || status === "stopped" || status === "trashed");
                getPauseBtn(campaignId).toggleClass("disabled", status === "paused" || status === "stopped" || status === "trashed");
                getArchiveBtn(campaignId).toggleClass("disabled", status === "stopped" || status === "trashed");
                getDeleteBtn(campaignId).toggleClass("disabled", status === "trashed");
                getUnarchiveBtn(campaignId).toggleClass("disabled", status !== "stopped" && status !== "trashed");
            };

            var updateCampaignStatusView = function(id, status) {
                var elem;
                if (status.toLowerCase() == "trashed") {
                    elem = $("#vwo-campaign-" + id);
                    elem.parentNode.removeChild(elem);

                    return;
                }

                elem = $("#vwo-campaign-status-" + id);
                elem.parentNode.removeChild(elem);

                var infoElem = $("#vwo-campaign-info-" + id);
            ...
            var toggleCampaignDetail = function(campaignId) {
                $("#vwo-campaign-" + campaignId).toggleClass("expanded");
                var detailsEl = $("#vwo-campaign-details-" + campaignId);
                detailsEl.toggleClass("hidden");
            };


            return {
                toggleCampaignDetails: function(campaignId) {
                    var expandedCampaingsNodeList = $(".vwo-campaign.expanded");
                    ...
                    if (campaignDetailsStore.hasOwnProperty(campaignId)) { // already fetched & rendered
                        ...
                        $("#vwo-campaign-" + campaignId).addClass("expanded");
                    }
                }
            }
        }();
        ...
        return {
            ...
            showMask: function() {
                $("#vwo-widget-loadmask").css('display', 'block');
            },

            hideMask: function() {
                $("#vwo-widget-loadmask").css('display', 'none');
            },
    ...
    $(document).ready(function() {
        ...
        $('#vwo-new-campaign-btn').on('click', vwo.toggleNewCampaignList);

        $('[name="campaignType"]').on('change', vwo.toggleCampaignWizard);
        $('.vwo-new-campaign-logo').each(index => vwo.toggleCampaignWizard(index));
        $('.vwo-new-campaign-name').each(index => vwo.toggleCampaignWizard(index));
